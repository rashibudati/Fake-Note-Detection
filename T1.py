...
xtr,xte,ytr,yte = getEmbeddings("/home/rashi/Desktop/task_NUS/Fake-news-Detection-master/datasets/train.csv")
np.save('./xtr', xtr)
np.save('./xte', xte)
np.save('./ytr', ytr)
np.save('./yte', yte)
...
#    model.add(Dense(256, activation='relu', kernel_initializer='normal'))
    ...
    # gradient descent
    sgd = SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True)   #SGC with momentum to accelerate the convergence
